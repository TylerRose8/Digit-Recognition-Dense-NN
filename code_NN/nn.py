...
class NeuralNetwork:
    ...
    def _init_weights(self):
        ...
        for i in range(1, self.L + 1):
            ...
            self.layers[i].W = weight_rng.uniform(-1 / math.sqrt(self.layers[i].d), 1 / math.sqrt(self.layers[i].d), (self.layers[i - 1].d + 1, self.layers[i].d))
    ...
    def _feed_forward(self, X, Y=None):
        ...
        if Y is not None:
            return np.sum(np.square(self.predict(X) - np.argmax(Y, axis=1))) / Y.shape[0]
        ...
        if SGD:
            ...
            for itr in range(iterations):
                ...
                for i in range(batch_X.shape[0]):
                    error = self._feed_forward(batch_X[i], batch_y[i])
                    # error = np.sum(np.square(self.layers[self.L].X - Y)) * 1 / (2 * len(X))
                    errors.append(error)
                    # error2 = self.error(X, Y)
                    # errors2.append(error2)
                    print('error: ', error)
                    self._back_propagation(batch_y[i], batch_X.shape[0])
    ...
    def error(self, X, Y):
        ...
        error = np.sum(np.square(self.predict(X) - np.argmax(Y, axis=1)))
